...
func buildCreateNetworkParams(opts CreateOptions, wireInterception *api.NetworkInterceptionConfig) map[string]interface{} {
	...
	includeNetwork := hasAllowedHosts || hasAddHosts || hasSecrets || hasDNSServers || hasHostname || hasMTU || hasNoNetwork || hasBlockPrivateIPsOverride || hasForceInterception || hasNetworkInterception || hasAllowedPrivateHosts || hasLocalModelRoutes || hasPlugins
	if !includeNetwork {
		return nil
	}
